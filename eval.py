...
def validate(opts, model, loader, device, metrics):
    ...
    with torch.no_grad():
        for i, (images, labels, _, _) in enumerate(tqdm(loader)):
            ...
            # feature_tsne = feature_tsne.transpose(0,2,3,1).reshape(1, -1, 256)
            # # np.savetxt('tsne/feature_tsne/feature_tsne_%04d.csv'%(i+1), feature_tsne[0], delimiter=' ')
            # label_tsne = labels.cpu().numpy()
            # label_tsne = np.resize(label_tsne,(1,513,513))
            # label_tsne = label_tsne.reshape(1,-1)
            # np.savetxt('tsne/label_tsne.csv', label_tsne[1], delimiter=' ')
            ...
            # save_preds = Image.fromarray(np.uint8(preds[0]))
            # save_preds.save('111.jpg')
