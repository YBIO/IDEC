...
def validate(opts, model, loader, device, metrics):
    ...
    with torch.no_grad():
        for i, (images, labels, _, _) in enumerate(tqdm(loader)):
            ...
            save_preds = convert_from_color_segmentation(preds[0]) 
...
def main(opts):
    ...
    report_dict[f'test_all_miou'] = test_score['Mean IoU']

    class_iou = list(test_score['Class IoU'].values())

    first_cls = len(get_tasks(opts.dataset, opts.task, 0)) 

    report_dict[f'before_mIoU'] = np.mean(class_iou[:first_cls]) 
    report_dict[f'after_mIoU'] = np.mean(class_iou[first_cls:])  
